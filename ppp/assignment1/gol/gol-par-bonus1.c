...
void ParallelWorldTimeStepOnRow(struct PartialParallelWorld *old_world,
...
void ParallelWorldStart(int world_rows, int world_cols, int nsteps,
    ...
    for (world_iter = 1; world_iter < nsteps; world_iter++) {
        ...
        // 2.1 wrap the world
        BorderWrap(world);
        // 2.2 calculate the new world for the edge row
        ParallelWorldTimeStepOnRow(world, next_world, 1);
        if (world->actual_row_num_ - 2 != 1) {
